...
Most Tweets posted on twitter are also character strings.

```{r}
## E.g what guys are posting now concerning floods on twitter in South Africa
library(rtweet)
rds <- search_tweets("floods")
rds$text
```
...
```{r}
rds1 <- search_tweets("bitcoin")
rds1$text

```
```{r}
rds2 <- search_tweets("Johnnydepps")
rds2$text

```

```{r}
rds3 <- search_tweets("datasciences")
rds3$text
```

```{r}
rds4 <- search_tweets("health information systems")
rds4$text
...
Data frames are a special case of _lists_. We will cover lists in more detail later but know that they are useful because you can store any combination of other types. Here is an example of a list we created for you:
